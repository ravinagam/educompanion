...
export async function POST(_req: Request) {
  ...
  const subjectSnapshots = (subjects ?? []).map(subj => {
    ...
    return {
      ...
      flashcards_known: subjFlashcards.filter((fp: any) => fp.status === 'known').length,
  ...
  const allScores = (quizAttempts ?? []).filter((a: any) => a.total > 0).map((a: any) => (a.score / a.total) * 100);
